...
function firstOrOther(){
    if($('#isComAch').val()=='true'){
        $('#unitInfo').show();
        //$('#unitTable').bootstrapTable("load",medunitTemps);
...
//显示详情
function showForm(){
    //$('#changeTable-box').addClass('collapsed');//暂时用于测
    $('#change-box').removeClass('collapsed');
}
//显示总览
function showTable(){
    $('#ChangeTable').bootstrapTable('refresh',{silent: true});
